...
1. `packages/shared-types`의 DTO 및 `docs/PRD-*.md` 요구사항을 리뷰한다.
2. NestJS 모듈/컨트롤러/서비스를 생성 또는 수정한다.
3. 접근 제어용 Guard가 누락되지 않았는지 이중 체크한다.
4. 단위 테스트 또는 curl을 통한 엔드포인트 검증 후 결과를 보고한다.
